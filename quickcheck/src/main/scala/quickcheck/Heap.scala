...
trait BinomialHeap extends Heap {
  ...
  protected def root(t: Node): A =
    t.x

  protected def rank(t: Node): Rank =
    t.r
  ...
  protected def ins(t: Node, ts: H): H = ts match {
    case Nil => List(t)
    case tp :: ts2 => // t.r<=tp.r
      if (t.r < tp.r) t :: tp :: ts2 else ins(link(t, tp), ts2)
  }

  override def empty = Nil

  override def isEmpty(ts: H): Boolean =
    ts.isEmpty
  ...
  override def meld(ts1: H, ts2: H): List[Node] = (ts1, ts2) match {
    ...
    case (t1 :: ts11, t2 :: ts22) =>
      if (t1.r < t2.r) t1 :: meld(ts11, t2 :: ts22)
      else if (t2.r < t1.r) t2 :: meld(t1 :: ts11, ts22)
      else ins(link(t1, t2), meld(ts11, ts22))
  }
